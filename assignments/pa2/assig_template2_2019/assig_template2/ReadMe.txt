...
Version 3.1     Mar.2018
Version 3.0	Oct.2009

Note!
This template is for COMP3271 course students only.
...
o Support JPG and BMP as textures/bump maps
...
importing textures/bump maps and exporting rendered image. The JpegLib
...
<CRayView> class
	...
	// a flag indicates if antialiasing is needed because there is a switch
	// to turn anti-aliasing on and off.
	...
	void	XProduct(float*,float*,float*);
...
<CRayDoc> class
	...
	float	m_flViewPoint[3];
	// viewpoint position

	CObList* m_pObjectList;
	// a CObList list, contains CPrimitive objects

	CObList* m_pLights;
	// a CObList list, contains CLightsource objects

	CObList* m_pTextures;
	// a CObList list, contains CTexture objects, for bump or texture map
...
<CPrimitive> class
	...
	// material setting
	float	m_flAmbient[4];		// ambient component, amb[3] not-used
	float	m_flDiffuse[4];		// diffuse component, dif[3] not-used
	float	m_flSpecular[4];	// specular component, dif[3] not-used
	float	m_flShininess;		// shininess range 1-256, 1-128 in OpenGL
	float	m_flRefrIndex;		// refractive index > 1.0
	float	m_flReflectance;	// range 0-1
	float	m_flOpacity;		// range 0-1
...
<CVertex> class
	...
	float	m_flX,m_flY,m_flZ;	// coordinate
	float	m_fls,m_flt;		// texture coordinate
	float	m_flnx,m_flny,m_flnz;	// normal

<CLightSource> class
	CLightSource class. variables include:

	float	m_flX,m_flY,m_flZ;		// light position
	float	m_flR,m_flG,m_flB;		// light color

<CTexture> class
	CTexture objects either store color texture or bump map. Only one useful
	function:
...
8. Add antialiasing -> image looks better and smoother.
	Before you start implementing antiliasing, make sure your basic ray
	tracing routines works. Try to render some big files and see if it is
	stable enough. Here big file means something you are going to trace for
	a few hours. It is because in antialiased ray tracing, the number
	of rays fired might be a few times more than that with aliasing.
	If your ray tracer has memory leak or wrong pointer assignment. You
	may not notice it with small scene but when you	are implementing
	antialiasing, you might get into trouble. If that happens, you might
	not know which part went wrong, the implementing anti-aliasing? or
	the old ray tracing part? that makes debugging very hard.
	Use the zoom mode to see the antialiasing effect clearly.
9. Ray trace 3 images each no smaller than 640x480 pixels using your own ray
...
Please send pretty scene files to bcgong@cs.hku.hk.
